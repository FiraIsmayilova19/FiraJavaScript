let goods = [
  ...
  const priceFilter=goods.filter(checkCost)
  function checkCost(goods){
    return goods.product_price<200 &&  50<goods.product_price
  }

  console.log(priceFilter)
  const nameFilter=priceFilter.filter(checkName)
  function checkName(goods){
    var text = "Qadın"
    return goods.product_name.includes(text)
  }
console.log(nameFilter)
for (let i=0 ; i<nameFilter.length;i++){
  let li = document.createElement('li')
  li.innerHTML = `
<li>${nameFilter[i].product_name}</li>
<li>${nameFilter[i].product_description}</li>
<li>${nameFilter[i].product_price}</li>
<li>${nameFilter[i].store_name}</li>
<li>${nameFilter[i].store_address}</li>
<li>${nameFilter[i].id}</li>
`
